...
export default function InstitutionalDashboard() {
  ...
  useEffect(() => {
    ...
    const profileReq = axios
      ...
      .catch((err) => {
        if (err.response?.status === 401) { localStorage.clear(); navigate("/login-institution"); }
  ...
  const handleScholarshipSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        remainingSeats: form.remainingSeats ? Number(form.remainingSeats) : undefined,
        locationFilter: {
          province:     form.provinceName    ? { provinceName: form.provinceName }       : undefined,
          district:     form.districtName    ? { districtName: form.districtName }       : undefined,
          municipality: form.municipalityName ? { municipalityName: form.municipalityName } : undefined,
        },
      };
      ...
      <div className="flex flex-1 overflow-hidden max-w-7xl mx-auto w-full px-4 sm:px-6 py-6 gap-6">
        ...
        <main className="flex-1 min-w-0 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Scholarships", value: scholarships.length, accent: true },
              ...
              >
                <p className={`text-2xl font-bold ${accent ? "text-blue-600" : "text-gray-800"}`}>
                  {value}
                </p>
          ...
          {tab === "scholarships" && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <h2 className="text-base font-bold text-gray-900">Scholarships</h2>
                <button
                  onClick={() => { setShowScholarshipForm((v) => !v); setSchError(""); }}
              ...
              {showScholarshipForm && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  <h3 className="font-bold text-gray-900 text-lg mb-1">New Scholarship</h3>
                  ...
                  <form onSubmit={handleScholarshipSubmit}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="sm:col-span-2">
                        <label className={labelCls}>Scholarship Title <span className="text-red-400">*</span></label>
                        <input className={inputCls} required value={form.scholarshipTitle} onChange={set("scholarshipTitle")} placeholder="e.g. Merit Scholarship 2025" />
                      </div>
                      <div className="sm:col-span-2">
                        <label className={labelCls}>Description</label>
                        <textarea className={inputCls + " resize-none"} rows={3} value={form.description} onChange={set("description")} placeholder="Describe the scholarship and its purpose..." />
                      </div>
                      <div className="sm:col-span-2">
                        <label className={labelCls}>Terms & Conditions</label>
                        <textarea className={inputCls + " resize-none"} rows={3} value={form.termsAndConditions} onChange={set("termsAndConditions")} placeholder="Any terms and conditions applicants should be aware of..." />
                      </div>
                      <div>
                        <label className={labelCls}>Application Deadline <span className="text-red-400">*</span></label>
                        <input className={inputCls} type="date" required value={form.applicationDeadline} onChange={set("applicationDeadline")} />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <input className={inputCls} type="number" min="0" value={form.amountNpr} onChange={set("amountNpr")} placeholder="e.g. 50000" />
                      </div>
                      <div>
                        <label className={labelCls}>Coverage Percentage (%)</label>
                        <input className={inputCls} type="number" min="0" max="100" value={form.percentage} onChange={set("percentage")} placeholder="e.g. 100" />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input className={inputCls} type="number" min="1" value={form.totalSeats} onChange={set("totalSeats")} placeholder="e.g. 10" />
                      </div>
                      <div>
                        <label className={labelCls}>Remaining / Available Seats</label>
                        <input className={inputCls} type="number" min="0" value={form.remainingSeats} onChange={set("remainingSeats")} placeholder="Defaults to Total Seats if left empty" />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <input className={inputCls} value={form.targetFaculty} onChange={set("targetFaculty")} placeholder="e.g. Science, Management" />
                      </div>
                      <div>
                        <label className={labelCls}>Subject</label>
                        <input className={inputCls} value={form.subject} onChange={set("subject")} placeholder="e.g. Computer Engineering" />
                      </div>
                      <div>
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <textarea className={inputCls + " resize-none"} rows={2} value={form.additionalRequirements} onChange={set("additionalRequirements")} placeholder="Any other eligibility details..." />
                      </div>
                      <div className="sm:col-span-2">
                        <label className={labelCls}>Required Documents</label>
                        <input className={inputCls} value={form.requiredDocuments} onChange={set("requiredDocuments")} placeholder="Comma-separated: slc_marksheet, plus2_gradesheet, citizenship" />
                        <p className="text-xs text-gray-400 mt-1">Separate multiple documents with a comma.</p>
                      </div>
                      <div className="flex items-center gap-3">
                        <input id="isNepali" type="checkbox" className="w-4 h-4 accent-red-500" checked={form.isNepali} onChange={set("isNepali")} />
                        <label htmlFor="isNepali" className="text-sm text-gray-700">Nepali citizens only</label>
                      </div>
                      <div className="flex items-center gap-3">
                        <input id="hasDisability" type="checkbox" className="w-4 h-4 accent-red-500" checked={form.hasDisability} onChange={set("hasDisability")} />
                        <label htmlFor="hasDisability" className="text-sm text-gray-700">For students with disability</label>
                      </div>
                    </div>

                    <SectionHeading>
                      Location Filter{" "}
                      <span className="normal-case font-normal text-gray-400">(leave blank = open to all)</span>
                    </SectionHeading>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        <label className={labelCls}>Province</label>
                        <input className={inputCls} value={form.provinceName} onChange={set("provinceName")} placeholder="e.g. Bagmati" />
                      </div>
                      <div>
                        <label className={labelCls}>District</label>
                        <input className={inputCls} value={form.districtName} onChange={set("districtName")} placeholder="e.g. Kathmandu" />
                      </div>
                      <div>
                        <label className={labelCls}>Municipality</label>
                        <input className={inputCls} value={form.municipalityName} onChange={set("municipalityName")} placeholder="e.g. Kathmandu Metropolitan" />
                      </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-100">
                      <button
                        type="button"
                        onClick={() => { setShowScholarshipForm(false); setForm(EMPTY_FORM); setSchError(""); }}
                    ...
                    <>
                          ...
                          >
                            <div className="flex items-start justify-between gap-2 mb-2">
                              ...
                              {s.coverage?.scholarshipType2 && (
                                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${TYPE_COLORS[s.coverage.scholarshipType2] || "bg-gray-100 text-gray-600"}`}>
                            ...
                            <div className="space-y-1.5 mb-4">
                              ...
                              {s.eligibilityCriteria?.targetLevel && (
                                <p className="text-xs text-gray-500 flex items-center gap-1.5">
                                  <span>🎓</span> {s.eligibilityCriteria.targetLevel.replace("_", " ")}
                                </p>
                              )}
                              <p className="text-xs text-gray-500 flex items-center gap-1.5">
                                <span>📝</span> {s.statistics?.totalApplications || 0} applications
                              </p>
                            </div>
                      ...
                      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div className="px-5 py-3 border-b border-gray-50">
                          <h3 className="text-sm font-semibold text-gray-700">All Scholarships</h3>
                        </div>
                        <table className="w-full text-sm">
                          <thead className="bg-gray-50 border-b border-gray-100">
                            <tr>
                              <th className="text-left px-5 py-2.5 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Title</th>
                              <th className="text-left px-4 py-2.5 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Type</th>
                              <th className="text-left px-4 py-2.5 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Deadline</th>
                              <th className="text-left px-4 py-2.5 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Seats</th>
                              <th className="text-left px-4 py-2.5 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Applications</th>
                          ...
                          <tbody className="divide-y divide-gray-50">
                            {scholarships.map((s) => (
                              <tr key={s._id} className="hover:bg-gray-50 transition-colors">
                                <td className="px-5 py-3 font-medium text-gray-900 text-sm">{s.scholarshipTitle}</td>
                                <td className="px-4 py-3">
                                  {s.coverage?.scholarshipType2 ? (
                                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${TYPE_COLORS[s.coverage.scholarshipType2] || "bg-gray-100 text-gray-600"}`}>
                                      {s.coverage.scholarshipType2.replace("_", " ")}
                                    </span>
                                  ) : <span className="text-gray-300">—</span>}
                                </td>
                                <td className="px-4 py-3 text-xs text-gray-500">
                                  {new Date(s.applicationDeadline).toLocaleDateString("en-NP", { day: "numeric", month: "short" })}
                                </td>
                                <td className="px-4 py-3">
                                  <SeatsBar remaining={s.remainingSeats} total={s.totalSeats} />
                                </td>
              ...
              {applications.length === 0 ? (
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-100">
                      <tr>
                        <th className="text-left px-5 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Student</th>
                        <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Scholarship</th>
                        <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Type</th>
                        <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Status</th>
                        <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-50">
                      {applications.map((app) => (
                        <tr key={app._id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${TYPE_COLORS[app.applicationType] || "bg-gray-100 text-gray-600"}`}>
                              {app.applicationType}
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${STATUS_COLORS[app.applicationStatus] || "bg-gray-100 text-gray-500"}`}>
                          ...
                          <td className="px-4 py-3">
                            {app.applicationStatus === "pending" && (
                              <div className="flex gap-1.5">
                                <button onClick={() => handleReview(app._id, "approved")}     className="text-[10px] px-2 py-1 bg-green-50 text-green-700 rounded-md hover:bg-green-100 font-medium">Approve</button>
                                <button onClick={() => handleReview(app._id, "under_review")} className="text-[10px] px-2 py-1 bg-blue-50 text-blue-700 rounded-md hover:bg-blue-100 font-medium">Review</button>
                                <button onClick={() => handleReview(app._id, "rejected")}     className="text-[10px] px-2 py-1 bg-red-50 text-red-700 rounded-md hover:bg-red-100 font-medium">Reject</button>
                              </div>
                            )}
        ...
        {sidebarOpen && (
          <aside className="w-72 shrink-0 bg-white rounded-2xl border border-gray-100 shadow-sm self-start sticky top-20 overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50">
              <span className="text-xs font-semibold text-gray-700 uppercase tracking-widest">Profile</span>
              <button
            ...
            <div className="flex flex-col items-center py-5 px-4 border-b border-gray-50">
              ...
              <p className="text-sm font-bold text-gray-900 text-center">{data?.institutionName}</p>
              <p className="text-[10px] text-gray-400 mt-0.5">{data?.institutionType}</p>
              {data?.establishedYear && (
                <p className="text-[10px] text-gray-400">Est. {data.establishedYear}</p>
              )}
            </div>
            ...
            <div className="p-3 space-y-1.5">
              ...
              <ExpandSection title="Location">
                ...
                {loc.municipality && <InfoRow label="Municipality" value={loc.municipality} />}
